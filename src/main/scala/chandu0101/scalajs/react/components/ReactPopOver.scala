...
object ReactPopOver {
  ...
  class Backend($ : BackendScope[Props, State]) {
    ...
    def getPosition(P: Props)(node: html.Element): ClientRect = {
      ...
      val popoverHeight = $.getDOMNode.map(_.asMounted.asElement).map(_.domAsHtml.offsetHeight).runNow()
      val popoverWidth  = $.getDOMNode.map(_.asMounted.asElement).map(_.domAsHtml.offsetWidth).runNow()
      P.placement match {
        case "right" =>
